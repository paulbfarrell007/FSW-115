...
function createToDo(data) {
    ...
    for (let i = data.length - 1; i >= 0; i--) {
        ...
        checkbox.addEventListener('click', e => {
            ...
            if (e.target.checked) {
                ...
                axios.put(`http://api.bryanuniversity.edu/paulfarrell/list/${parentNode.id}`, { isComplete: true })
                    .then(res => console.log(res))
                    .catch(err => console.log(err))
            } else {
                e.target.parentNode.style.textDecoration = 'none';
                axios.put(`http://api.bryanuniversity.edu/paulfarrell/list/${parentNode.id}`, { isComplete: false })
                    .then(res => console.log(res))
                    .catch(err => console.log(err))
            }
        });
        ...
        deleteButton.addEventListener('click', (e) => {
            ...
            axios.delete(`http://api.bryanuniversity.edu/paulfarrell/list/${toDelete.id}`)
                .then(res => getData())
                .catch(err => console.log(err))
        });
        updateButton.addEventListener('click', (e) => {
            ...
            axios.put(`http://api.bryanuniversity.edu/paulfarrell/list/${itemId.id}`)
                .then(res => getData())
                .catch(err => console.log(err))
        });
    };
    ...
    todoForm.addEventListener('submit', e => {
        ...
        axios.post(`http://api.bryanuniversity.edu/paulfarrell/list/`, newTodo)
            .then(res => getData())
            .catch(err => console.log(err))
    });
    ...
    newData.addEventListener('submit', e => {
        ...
        axios.put(`http://api.bryanuniversity.edu/paulfarrell/list/${itemId}`, updateTodo)
            .then(res => getData())
            .catch(err => console.log(err))
    });
};
